...
impl<Progress: Write> DependencyGraphBuilder<Progress> {
    ...
    pub fn get_graph(
        ...
        let root_pkg_name = custom_resolve_pkg_name(&root_path, &root_manifest).with_context(|| {
            format!(
        ...
        println!(
            "{} -> {}",
            root_manifest.package.name.to_string(),
            root_pkg_name
        );
        ...
        // get overrides
        let overrides = collect_overrides(parent, &resolved_name_deps)?;
    ...
    fn collect_graphs(
    ...
    ) -> Result<(BTreeMap<PM::PackageName, DependencyGraphInfo>, PM::Dependencies)> {
        ...
        /*
        println!(
            "parent pkg: {}, root_path: {}, dependencies: {:#?}, resolved_deps: {:#?} dep_graphs: {:#?}",
            parent_pkg,
