...
pub trait PackageHooks {
    ...
    fn custom_resolve_pkg_name(&self, pkg_path: &PathBuf, manifest: &SourceManifest) -> anyhow::Result<Symbol>;
}
...
pub(crate) fn custom_resolve_pkg_name(pkg_path: &PathBuf, manifest: &SourceManifest) -> anyhow::Result<Symbol> {
    if let Some(hooks) = &*HOOKS.lock().unwrap() {
